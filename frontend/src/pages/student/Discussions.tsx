  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="bg-slate-50 border-dashed text-center py-8">
        <MessageSquare className="mx-auto text-slate-400 mb-3" size={40} />
        <p className="text-sm font-bold text-navy-900">Discussion forum is under development</p>
        <p className="text-xs text-slate-500 mt-1">Browse placeholder threads below.</p>
      </Card>

      <div className="space-y-4">
        {PLACEHOLDER_THREADS.map((thread) => (
          <Card key={thread.id} className="flex items-start justify-between gap-4 hover:border-slate-300 cursor-pointer transition-colors">
            <div className="flex gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <h3 className="font-extrabold text-navy-900 text-sm truncate">{thread.title}</h3>
                  {thread.pinned && <Badge variant="warning">Pinned</Badge>}
                </div>
                <p className="text-xs text-slate-500 mt-0.5">{thread.course} • by {thread.author}</p>
              </div>
            </div>
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-[10px] text-slate-400 mt-1">{thread.lastActive}</p>
            </div>
          </Card>
        ))}
      </div>
